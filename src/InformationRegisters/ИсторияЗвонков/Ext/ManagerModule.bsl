...
//  ДобавлятьСобытия - булево - флаг для добавления/перезаписи данных - определяем поле "Период"
//
Процедура СоздатьЗапись( соотвПолейЗвонков, ДобавлятьСобытия = Истина ) Экспорт 
	...
	Для каждого эл Из соотвПолейЗвонков Цикл
		Если эл.Ключ = "ЗвонящийНомер" 
			И эл.Значение = "" И рс[эл.Ключ] <> "" Тогда
			Продолжить; // +++ 01.08.2022 - НЕ затираем вн.номер,
						// если в регистре он есть, а в новых данных - нет
		КонецЕсли;	
	...
	рс.Записать(Истина); // перезапись ВСЕГДА!
