	...
	Иначе // --------------------------------------------------------   
		...
		Иначе	  
			Для каждого эл Из стрЗвонка Цикл 
				Если эл.Ключ = "Канал"  // +++ 02.08.2022 - НЕ затираем Канал =Вн.Номер,
					// если в регистре он есть, а в новых данных - нет (Потерянный исх. звонок)
					И стрЗаписи[эл.Ключ] <> "" И эл.Значение = "" Тогда
					Продолжить; 						
				КонецЕсли;	
...
//
// Параметры:
//  Запрос	 - HttpЗапрос - входящий запрос важны заголовки!
// 
// Возвращаемое значение:
//  строка - текст для Браузера в кодировке ANSI (Win-1251)
//
Функция ПингGET(Запрос)
	ТелоОтвета = "HTTP-Сервис работает! Its Work!";
	
	Ответ = Новый HTTPСервисОтвет(200, "");
	Ответ.УстановитьТелоИзСтроки(ТелоОтвета, КодировкаТекста.ANSI); // 01.08.2022 - для браузера!
